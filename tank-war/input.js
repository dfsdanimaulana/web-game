export default class InputHandler {
  ...
  handleSwipe() {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      // Horizontal swipe
      if (dx > 0) {
        this.keys.indexOf("ArrowRight") === -1 && this.keys.push("ArrowRight");
        this.previousSwipeDirection = "right";
      } else {
        this.keys.indexOf("ArrowLeft") === -1 && this.keys.push("ArrowLeft");
        this.previousSwipeDirection = "left";
      }
    } else {
      // Vertical swipe
      if (dy > 0) {
        this.keys.indexOf("ArrowDown") === -1 && this.keys.push("ArrowDown");
